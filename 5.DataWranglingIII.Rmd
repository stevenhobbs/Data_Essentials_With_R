---
title: "5. DATA WRANGLING III"
subtitle: "Mutate & ifelse"
output:
...
Using mutate to change a dataframe during the initial import can also reduce errors and knitting failures. This strategy results in a single version of a dataframe that is used throughout the analysis. Consider that rarely do we write a script perfectly the first time. Often we return to earlier sections of a document to add or change code. Jumping around within a script that depends on many different versions of a dataframe allows us to change early code that relies on a dataframe produced by later code. The code may initially run fine, but will fail to knit and will throw errors for new sessions of R when we attempt to run the code sequentially. By creating one version of a dataframe during the initial import, we avoid problems related to editing and running code out of sequence.

The code below imports our data, recodes the sex and group variables and then adds 6 more variables all in one execution. The code may look intimidating at first, but consider that it does not need to be written all at once, and probably shouldn't be. A less daunting approach is to start small, perhaps just with the read_csv command and then iteratively add a component, execute the code and check the output. 
...
df <- 
    ...
    mutate(sex = fct_recode(sex, 'male' = '0', 'female' = '1'),
           ...
           # while still inside the mutate command, we create add 6 more variables
