...
# Now the values are named. This is truly unnecessary because we have a separate column for names.
attributes(age)  

# Don't confuse data with metadata. Even though age now has a name attribute, the age variable is still an integer variable. The data are numeric.
2*age 
```
